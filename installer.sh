...
while [[ $is_installed_apps != "Y" && $is_installed_apps != "y"]]; do
    read -p "Have you already installed all the applications?:(Y/n) " is_installed_apps
done
...
proot-distro login debian --shared-tmp -- bash -c "apt install locales fonts-noto-cjk -y && dpkg-reconfigure locales && locale"
...
if [[ ! -d $sdk_tools_folder ]]; then
    ...
    echo "Debian ENV: $sdk_tools_folder exist, skipping..."
...
if [[ $is_fixes_mpp == "Y" || $is_fixes_mpp == "y" ]]; then
    ...
    echo "Skipping the max_phantom_processes fix."
fi
